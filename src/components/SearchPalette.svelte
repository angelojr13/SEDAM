...
{#if $nimboStore.showCommandPalette }
  <div use:init on:keyup|stopPropagation class="absolute w-full top-0 left-0 h-screen z-10 bg-dark-300-75" transition:fade={{ duration: 100 }}>
    <div class="bg-light-200 dark:bg-dark-200 mt-24 w-3/4 md:w-2/3 xl:w-1/2 mx-auto rounded py-2 shadow-lg" use:handleClickOutside>
      <div class="px-2">
        ...
        <input id="cmdInput" class="dark:text-dark dark:bg-dark-300 text-light text-2xl pl-12 pr-2 py-2 w-full bg-light-300 border border-transparent border-indigo-500 appearance-none leading-normal rounded outline-none" bind:value={query} on:keyup={handleSearchInput} type="text">
      </div>
        ...
        {#each searchResults as s}
          <li class="result px-2 py-1 font-bold text-light dark:text-dark text-lg cursor-pointer focus:font-bold focus:bg-light-100 dark:focus:bg-dark-100 hover:bg-light-100-50 dark:hover:bg-dark-100-50 outline-none truncate" on:keydown={e => handleResultsEvent(e, s) } on:click={e => execAction(s) } tabindex="0">
            {#if s.obj.type == RESULT_TYPE.BOARD}
              <span class="rounded border border-2 px-2 uppercase text-sm mr-1 text-green-500 border-green-500 dark:text-green-300 dark:border-green-300">Tablero</span>
            {:else if s.obj.type == RESULT_TYPE.CARD}
              <span class="rounded border border-2 px-2 uppercase text-sm mr-1 text-red-500 border-red-500 dark:text-red-300 dark:border-red-300">Tarjeta</span>
            {/if}
            {#if s.target}
