...
<div class="flex items-center px-1 bg-light-100 dark:bg-dark-200 text-light dark:text-dark rounded" on:keyup|stopPropagation class:completed={complete}>
  ...
  {/if}
  <input id="flatpickr" class="bg-transparent py-1 pr-4 min-w-0 placeholder-light dark:placeholder-dark placeholder-opacity-100 outline-none" placeholder="Seleccione Fecha" />
  {#if !complete && date}
