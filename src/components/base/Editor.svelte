<script lang="ts">
  ...
  const handleDeleteCard = (e: Event): void => {
    dispatch("delete", {
      message: "¿Desea eliminar la tarjeta?",
      onConfirm: () => {
        dispatch("update", {
  ...
  <div class="mb-2">
    <div class="flex items-center">
      ...
      {#if editDescription || cardDetails.card.description }
        <div>
          <button on:click={handleDescriptionClick} class="ml-2 text-base underline dark:opacity-50 dark:hover:opacity-75"  >{editDescription ? "Guardar" : "Edit"}</button>
        </div>
      {/if}
    </div>    
    <div class="mt-2 text-lg" class:ml-8={!inline}>
      ...
      {:else}
        {#if editDescription}
          <textarea value={cardDescription} on:keyup|stopPropagation={handleDescriptionInput} class="w-full bg-white border-light-200 border-2 dark:border-transparent dark:bg-dark-100 p-1 rounded placeholder-dark-200 dark:placeholder-gray-400 shadow-sm" placeholder="Añadir descripción" rows="10"></textarea>
        {:else}
          <div class="opacity-75 hover:opacity-100 underline cursor-pointer" on:click={handleDescriptionClick}>Añadir descripición</div>
        {/if}
      {/if}
    </div>
  </div>
